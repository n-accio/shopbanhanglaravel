...
<body>
				...
				<div class="row">
					...
					<div class="col-sm-5">
						...
						<div class="search_box pull-right">
							...
							<input style="margin-top:0px;color:white" type="submit" class="btn btn-primary btn-sm" value="Tìm kiếm" />
						</div>
						</form>
					</div>
				</div>
						...
						<div class="carousel-inner">
							...
							@foreach ($slider as $key => $slide )
								...
								<div class="item {{$i==1 ? 'active' : ''}}">
									<div class="col-sm-12">
										<img alt="{{$slide->slider_desc}}" src="{{asset('public/uploads/slider/'.$slide->slider_image)}}" height="200px" width="100%" class="img img-responsive"  />
									</div>
								</div>
							@endforeach
	...
	{{-- <script src="{{asset('public/frontend/css/sweetalert.css')}}"></script> --}}
	
	
	
	{{-- // Sweet alert --}}

	<!-- Nut chia sẻ Facebook-->
	<div id="fb-root"></div>
	<script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v8.0" nonce="2GpndubG"></script>
	<!--/Nut chia sẻ Facebook-->
        ...
        $(document).ready(function(){
            $('.add-to-cart').click(function(){
				...
				if(parseInt(cart_product_qty)> parseInt(cart_product_quantity)){
					alert('Vui lòng đặt sản phẩm với số lượng <= ' + cart_product_quantity );
				}else{
					$.ajax({
	...
	<!-- Chọn địa chỉ để nhấn button tính phí vận chuyển-->
	<script type="text/javascript">
		$(document).ready(function(){
			$('.caculate_delivery').click(function(){
				...
				var _token = $('input[name="_token"]').val();
				if(matp==''|| maqh==''|| xaid=='' ){
					alert('Vui lòng chọn nơi vận chuyển để tính phí vận chuyển');
				}else{
					$.ajax({
	...
	<!-- Gửi thông tin đặt hàng để lưu khi kích chọn button xác nhận đơn hàng -->
	<script type="text/javascript">
        $(document).ready(function(){
            $('.send_order').click(function(){
				swal({
					title: "Xác nhận đơn hàng",
					text: "Đơn hàng sẽ được gửi về địa chỉ bạn đã điền",
					icon: "warning",
					 buttons: {
					...
					.then((value) => {
						switch (value) {
							...
							default:
								swal("Đóng", "Đơn hàng chưa được đặt", "error");
								break;
						}
					})
            });
