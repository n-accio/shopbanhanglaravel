...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on('blur','.fee_feeship_edit',function(){
			...
			var _token = $('input[name="_token"]').val();
			$.ajax({
				url : '{{url('/update-feeship-delivery')}}',
				method: 'POST',
		...
		//Chọn thành phố quận huyện để vận chuyển
...
<script type="text/javascript">
	$('.update_quantity_order').click(function(){
		...
		$.ajax({
				...
				method: 'POST',
				data: {order_product_id:order_product_id,order_qty:order_qty,order_code:order_code,
					_token:_token},
				success:function(data){
...
<script type="text/javascript">
	$('.update_status_order_details').change(function(){
		...
		for(i=0;i<order_product_id.length;i++){
			...
			if(parseInt(order_qty) > parseInt(order_qty_storage)){
				j = j+1;
				//Để thông báo chỉ hiện 1 lần
				if(j==1){
					alert('Số lượng hàng trong kho không đủ');
				}
				$('.color_qty_'+order_product_id[i]).css('background','#ddd');
			}
		}
		if(j==0){
			$.ajax({
...
<!-- calendar -->
	<script type="text/javascript" src="{{asset('puclic/backend/js/monthly.js')}}"></script>
	<script type="text/javascript">
